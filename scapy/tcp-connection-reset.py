...
def is_packet_on_tcp_conn_over_limit(client_ip, server_ip, server_port, limit):
    ...
    def f(p):
        ...
        will_block = is_packet_tcp_server_to_client and packet_counter > limit

        log_info(
            "Packet #" + str(packet_counter),
            {
...
if __name__ == "__main__":
    if (len(sys.argv) < 4):
        log_info("Usage: python ./tcp-connection-reset.py <client_ip> <server_ip> <server_port>")
        exit()
    ...
    t = sniff(
        iface=iface1,
        count=5000,
        prn=send_reset(iface1),
        lfilter=is_packet_on_tcp_conn_over_limit(client_ip, server_ip, server_port, 100)
        )
